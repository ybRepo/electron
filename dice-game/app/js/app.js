...
// Initialize game by setting starting values
function init() {
    scores = [0, 0];
    activePlayer = 0;
    roundScore = 0;
...
// Takes care of changes to game when user clicks Roll.
...
// Takes care of banking the cummulated turn score into global score when user clicks Hold.
...
function nextPlayer() {
    ...
    //Remove the dice 1s after the nextPlayer is called.
    setTimeout(function (){document.querySelector('.dice').style.display = 'none'}, 1000);
}

// Takes care of starting new game when button clicked by calling init function
